...
Create table line_err_multipoints as 
  ...
  from line_err;
Select RecoverGeometryColumn('line_err_multipoints', 'geometry', 31468, 'multipointz', 'xyz');
...
-- only to initialise the counters
 select 
  (select sum(z) / count(z) from line_err_points where rowid = 1 and z > 50 and z < 55) as mean_z, 
  0 as rid,
  NULL as z_new,
  -1 as item 
 union all
-- get the values or count + 1 if point_item does not exist 
 select
  case when (select z from line_err_points where rowid = rid and item_no = item + 1) not null
   then mean_z
   else (select sum(z) / count(z) from line_err_points where rowid = rid + 1 and z > 50 and z < 55)
   end as mean_z, 
  case when (select z from line_err_points where rowid = rid and item_no = item + 1) not null
   then rid
   else rid + 1
   end as rid, 
  case when (select z from line_err_points where rowid = rid and item_no = item + 1) > 50
...
-- do something
Select a.rid, b.line_feature_id, b.item_no, a.mean_z, a.z_new, b.z, b.x, b.y from point as a
