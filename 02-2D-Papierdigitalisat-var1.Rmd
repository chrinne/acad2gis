...
Im nächsten Schritt werden die zahlreichen Steine bzw. Knochen die je Datei als Multipolygon vorliegen getrennt. Die Anweisung unterscheidet sich deutlich von der vorangehenden Version für genau zwei Polygone in einem Multipolygon.
...
Danach erfolgt das Prüfen, Wiederherstellen und ggf. Korrektur der Geometriespalte.  
...
Wenn gewünscht noch die verarbeitetn Linien aus line_layer_2d löschen. Auch dies erfolgt mit Rekursion.

```
with recursive liste (id) as (
 select 1 
  union all
 select id + 1 from liste limit (select count(*) from tmp_Polygonize)
 )
delete from cp_line_layer_2d
 where feature_id in (
  select fids from tmp_Polygonize, liste 
   where tmp_Polygonize.rowid = liste.id)
```
