...
Dpalte "geometry" Kontextmenü > "Malfomed geometries". Liefer 495 fehlerhafte Geometrien: 1. überwiegend wiederholter Knoten (*repeated vertex*), 2. fehlerhafte Geometrie durch zu wenig Punkte.

1. Problem kontrollieren und reparieren:
```
...
```
```
Select astext(sanitizegeometry(geometry))
from line_layer_2d
where feature_id = 32
```

Der Startpunkt ist doppelt aber nicht identisch mit dem Endpunkt, also kein Ring bzw. in CAD nicht korrekt geschlossenes Polygon. Also erneut ausführen und [Repair] bestätigen.

Es bleiben drei fehlerhafte Geometrien (feature_id: 5570, 17618, 22049). Die erste besteht aus vier identischen Punkten. Wir kontrollieren das Problem und bereinigen es mit:
