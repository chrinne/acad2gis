...
Es folgt eine Datenkontrolle mit Überarbeitung, die vor allem den Transfer der fast ausschließlich als Linien erkannten Steine und Knochen betrifft. Die Befundlinien sind in den vorliegenden kleinen Planausschnitten stark zerstückelt und bilden einen Sonderfall. In "normalen" Fällen können Befundgrenzen analog zu dem hier an Steinen und Knochen durchgeführten Prozess behandelt werden. 
...
Es bleiben 78 fehlerhafte Geometrien mit "Pepeated vertex. Too few points in geometry ...".  Die Kontrolle der Geometrie von Feature 412 zeigt drei identische Punkte. Veralgemeinernd können wir mit folgender Abfrage diese fehlerhaften erst finden und dann auch löschen:
...
where length(geometry) = 0 and isvalid(geometry) = 0;
...
order by 3
```
Liefert 14567 Linien, dabei auch eine mit drei Punkten. Letztere ist ein Objekt ohne Fläche und kann gelöscht werden.

```
delete from line_layer_2d
where StartPoint(geometry) = EndPoint(geometry) 
 and numpoints(geometry) < 4
```
