...
Es handelt sich um eine durch ein Grabensystem befestigte spätneolithische Siedlung der Wartberg Gruppe. Nach einigen Prospektionen erfolgte 1988 eine erste Ausgrabung durch das Landesdenkmalamt Hessen, Außenstelle Marburg und 1990 eine zweite Ausgrabung durch die Universität Marburg, Fachbereich Vor- und Frühgeschichte (@fiedlerBefestigteSiedlungJungsteinzeit1991). Beide Grabungen wurden in der seinerzeit üblichen Form mit Theodolit vermessen und auf Papier dokumentiert. Für eine erneute Ausgrabung ab 2021 wurden alle alten Planzeichnungen in einer CAD-Zeichnung fortlaufend digitalisiert. Das alternative Modell, je Zeichenblatt eine Zeichnung und nachfolgende ergänzen zu 4 Gesamtzeichnungen für vier Plana wird am Bsp. Odagsen durchgeführt (s.u.).

Die CAD-Datei wurde in AutoCAD 2020 (Schulversion) und 2010 bearbeitet, sie beinhaltet einen entsprechenden Hinweis. Die Layerbezeichnungen der Zeichnung seit sehr normiert (s. Abb. 1) und es werden zahlreiche Blöcke auch mit Attributen verwendet.
...
| ... |  |  |
| PL03_BefundNr | 247 | Continuous |
| PL04_Befund | 245 | Continuous |
| ... |  |  |
...
**Wichtig** Innerhalb von Jedem Befund steht als Text und als Block mit Attribut eine Befundnummer, an jeder Profillinie steht steht ein Text mit der Bezeichnung des Profiles oder des jeweiligen Endpunktes. Dies Informationen werden in GIS verbunden.

## SpatiaLite GUI

Nach dem Start der GUI "Menu > Create a new (empty) SQLite DB". In diese wird die dxf-Datei mit der gesamten Ausgrabung importiert "Menu > Advanced > Import DXF drawings". Im Fenster zum Import folgende Angaben: (x) Import selected DXF drawing file only, SRID: 31467, [v] Append to already existing tables, Dimensions: (x) authomatic 2D/3D, (x) mixed layers, Special Rings handling (x) none. Diese Angaben beziehen sich auf den aktuellen Import und müssen ggf. angepasst werden. Die Option 'mixed layers' trennt nur die Typen (Punkt, Linie Polygon, Text), die Layer werden als Attribut (Spalte) angelegt. Die Alternative trennt erst die Layer und dann nach Typen, erstellt also das x-Fache der vorhandenen CAD-Layer als Tabellen.
...
select layer, geometrytype(geometry) as GeoType, count(*) as n 
from hatch_layer_2d
group by 1, 2
```

Die Syntax aggregiert auf die erste und zweite Spalte, den Layernamen und die Abfrage des Geometrietyps. Dazu liefert sie die Anzahl der Einträge. Warum einige Geometrien nicht vorliegen (NULL) kann ich nicht sagen. Im vorliegenden Fall handelt es sich durchgängig um eine Kreuzschaffur (ANSI37) für Tiergänge oder andere Störungen. 
...
Diese Abfrage liefert im vorliegenden Fall zwei Problemgruppen: Pl01_Steine hat elf und PL02_Befunde hat zwei Linienobjekte. In allen Fällen sollten das Polygone sein. Die Korrektur erfogt am besten in CAD: beim Layer Steine ist es ein Stein der aus zehn einzelnen Linien besteht und beim Layer Befunde sind es zwei nicht geschlossene Befunde. Die Grabungsgrenzen sind im vorliegenden Fall  aufgrund der Vorgaben heterogen sowohl als Polygon als auch als Linie vorhanden. Hier ist die jeweilige Fragestellung relevant: z.B. Fläche bemessen oder 1:1-Abbildung des Papiers.

Die Tabelle text_layer_3d_attr und die Sicht text_layer_3d_attr_view liefern in diesem Fall keine erhellenden Informationen und können wie hatch_layer_2d gelöscht werden.
...
Wird der vorangehenden Abfrage ein ```create table as Pl01_BefundeNr``` vorangestellt wird die entsprechende Tabelle erstellt. Nach einem *refresh* der Datebank ist diese Tabelle sichtbar. Die Angaben zur dort enthaltene Geometrie kann erst mit 'Check geometries' aus dem Kontextmenü überprüft und dann mit 'Recover gemetrie column' wieder hergestellt werden. Vergleichbare Vorgänge für jede Informationseinheit wiederholen.
...
Nun die Befundnummer mittels *spatial join* nachgeschlagen und in dem Feld 'number' eingeragen.
...
Nicht alles wird korrekt laufen. In QGIS die Beschriftung der Befunde einrichten,  kontrollieren und die Probleme von Hand analog der folgenden Anweisung  bereinigen. Die "Massenzuweisung" sind überwiegend die vielen einzelnen Befunde der tiefern Plana die nicht individuell benannt wurden. Optional zur Handarbeit über die Liste von ID's könnte auch mit einem großen Puffer um die Befundnummer (s.o.) gearbeitet werden. Allerdings sollte ein Pfosten in einem Grubenhaus besser eine neue Befundnummer erhalten oder die einzelnen Quadranten einer Grube im Planum 1 sollten doch eher zu einem Befund verschmolzen werden. Die sind  individuelle und situativ zu treffende Entscheidungen.
...
Im vorliegenden Fall sind die Tiergänge und andere Störungen schraffiert. Leider sind im vorliegenden Fall einige fehlerhafte Geometrien dabei. In der Spalte 'info' wird ein "TG" ergänzt wenn die feature_id in der Liste der Befunde mit einer innenliegenden Schraffur steht.
...
Optional für das schnelle Abfragen der Profilinfos in den Layereigenschaften von QGIS folgende "Anzeige" (pop ups, ballon, Sprechblase) bei den HTML-Kartenhinweisen definieren:
...
-- Hier wird der LINESTRING-WKT zu einem POLYGON-WKT umgeschrieben. 
select st_geometrytype(geomfromtext('POLYGON (' || 
 substr(st_astext(geometry),11) ||')')) as geo 
from line_layer_2d
...
select filename, layer, geomfromtext('POLYGON (' || 
 substr(st_astext(geometry),11) ||')', 31467) as geo 
from line_layer_2d
where feature_id=148;
...
**Wichtig**: AutoCAD verwendet den "." als Dezimaltrennzeichen, das "," ist der Spaltentrenner. Ein Text mit "," wie z.B. "210,15 m üNN" ohne eine Textauszeichnung ("<text>") führt zu einem Spaltenfehler.
