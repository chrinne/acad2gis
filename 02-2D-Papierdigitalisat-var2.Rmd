...
An dieser Stelle ein schnelle Kontrolle des Datenbestandes durch eine Tabellierung der Layer in jeder Tabelle
...
Diese Abfrage liefert im vorliegenden Fall zwei Problemgruppen: Pl01_Steine hat elf und PL02_Befunde hat zwei Linienobjekte. In allen Fällen sollten das Polygone sein. Die Korrektur erfogt am besten in CAD durch, vermutlich durch die Attributsangabe "geschlossen". Die Grabungsgrenzen sind im vorliegenden Fall  aufgrund der Vorgaben heterogen sowohl als Polygon als auch als Linie vorhanden.

Die Tabelle text_layer_3d_attr und die Sicht text_layer_3d_attr_view liefern keine erhellenden Informationen und können wie hatch_layer_2d gelöscht werden.
...
Wenn der Text neben dem Befund steht ist eine Option zunehmende Puffer um den Punkt des Textes (Befundnummer) zu rechnen und dann st_intersect zu nutzen. Sollte das Feld 'number' noch NULL sein wird es sucsessive gefüllt:
...
Nicht alles wird korrekt gelaufen. In QGIS die Beschriftung der Befunde einrichten,  kontrollieren und die Probleme von Hand mit analog der folgenden Anweisung  bereinigen.
